var IT={'Alegere duala':{code:'AF',lvl:1,pts:1,instr:'Identifica valoarea de adevar a afirmatiei de mai jos.'},'Alegere singulara':{code:'AS',lvl:1,pts:1,instr:'Selecteaza raspunsul corect din variantele propuse.'},'Alegere multipla':{code:'AM',lvl:1,pts:1,instr:'Selecteaza toate variantele corecte din lista de mai jos.'},'Tip pereche':{code:'TP',lvl:1,pts:1,instr:'Asociaza elementele din coloana A cu cele din coloana B.'},'Completare':{code:'C',lvl:2,pts:2,instr:'Completeaza spatiile libere cu informatia corecta.'},'Raspuns scurt':{code:'RS',lvl:2,pts:2,instr:'Scrie raspunsul corect (cuvant, expresie sau valoare numerica).'},'Structurat':{code:'Str',lvl:2,pts:2,instr:'Rezolva cerintele de mai jos, respectand pasii indicati.'},'Eseu structurat':{code:'Es',lvl:3,pts:4,instr:'Elaboreaza un raspuns argumentat respectand reperele indicate.'},'Probl. libere':{code:'PL',lvl:3,pts:4,instr:'Rezolva problema utilizand metoda potrivita si argumenteaza.'}};
function giftTemplate(c){switch(c){case 'AF':return'{TRUE}';case 'AS':case 'AM':return'{=Raspuns_corect ~Varianta_gresita_1 ~Varianta_gresita_2 ~Varianta_gresita_3}';case 'TP':return'{=Element_A -> Pereche_1 =Element_B -> Pereche_2}';case 'C':case 'RS':return'{=raspuns_corect}';default:return'{}';}};
var NC_COL={1:'#1565c0',2:'#2e7d32',3:'#bf360c'};
var NC_NM={1:'NC1 \u2013 Cunoastere',2:'NC2 \u2013 Aplicare',3:'NC3 \u2013 Analiza-Sinteza'};
var TIP_COL={Intrare:'#1565c0',Tematic:'#2e7d32',Intermediar:'#6a1b9a',Final:'#bf360c'};
var AUTO_CFG={a:2,s:10,m:4};
var DEFS={Intrare:{nc1:60,nc2:30,nc3:10,nrItemi:15,timp:20,prag:30},Tematic:{nc1:50,nc2:35,nc3:15,nrItemi:16,timp:35,prag:50},Intermediar:{nc1:30,nc2:40,nc3:30,nrItemi:30,timp:50,prag:70},Final:{nc1:20,nc2:30,nc3:50,nrItemi:30,timp:60,prag:70}};
var AUTO_BLOOM={1:['Alegere duala','Alegere singulara','Alegere multipla'],2:['Completare','Raspuns scurt'],3:['Eseu structurat']};
var testsList=[],giftRaw='',selMatKey=null,selSblKey=null,sblData={};
var rowIdx=0,curImpType='cg';
...
function doReset(){
  ...
  document.getElementById('pevW').innerHTML='<div class="es"><div class="ei">📅</div>Apasati ⚡ GENEREAZA</div>';
  document.getElementById('matTree').innerHTML='<div class="es"><div class="ei">📊</div>Generati</div>';
  document.getElementById('matC').innerHTML='<div class="es"><div class="ei">👈</div>Selectati test</div>';
  document.getElementById('sblTree').innerHTML='<div class="es"><div class="ei">📋</div>Generati</div>';
  document.getElementById('sblC').innerHTML='<div class="es"><div class="ei">👈</div>Selectati testul din stanga</div>';
  document.getElementById('colW').innerHTML='<div class="es"><div class="ei">📦</div>Generati</div>';
  document.getElementById('noteW').innerHTML='<div class="es"><div class="ei">📈</div>Generati</div>';
  document.getElementById('giftO').innerHTML='<span class="cm">// Apasati ⚡ GENEREAZA\u2026</span>';
  ...
  toast('\u2705 Sesiune resetata','#2e7d32');
}
function openImport(type){
  curImpType=type;var isCG=type==='cg';
  document.getElementById('importDlgTitle').textContent=isCG?'\ud83d\udcda Plan de lunga durata \u2014 Cultura Generala':'\ud83d\udd27 Curriculum Specialitate \u00cePT';
  document.getElementById('importDlgHint').innerHTML=isCG?
    '<b>Metoda 1:</b> Incarcati fisierul DOCX sau PDF.<br><b>Metoda 2:</b> Copiati tabelul <b>ADMINISTRAREA DISCIPLINEI</b> si lipiti mai jos.':
    '<b>Metoda 1:</b> Incarcati fisierul DOCX sau PDF.<br><b>Metoda 2:</b> Copiati tabelul <b>REPARTIZARE ORIENTATIVA A ORELOR</b> si lipiti mai jos.<br><span style="color:#2e7d32;font-weight:700">Coloane extrase: Unitati de invatare + Prelegeri (ore)</span>';
...
function runImportDlg(){
  ...
  if(!raw){toast('\u26a0 Lipiti textul sau incarcati un fisier!','#bf360c');return;}
  var found=curImpType==='cg'?parseCGImproved(raw):parseIPTImproved(raw);
  if(!found.length)found=parseUni(raw);
  if(!found.length){toast('\u26a0 Nu s-au detectat module. Verificati formatul.','#bf360c');return;}
...
async function extractModulesFromPDF(file){
  ...
  try{
    if(typeof pdfjsLib!=='undefined'){
      ...
      var found=curImpType==='cg'?parseCGImproved(combined):parseIPTImproved(combined);
  ...
  console.warn=_cw;console.error=_ce;console.log=_cl;
  try{var txt=await rdPdf(file);if(!txt)return[];var fb=curImpType==='cg'?parseCGImproved(txt):parseIPTImproved(txt);return fb.length?fb:parseUni(txt);}catch(e2){return[];}
}
async function hImportFile(file){
  if(!file)return;
  var stat=document.getElementById('impFileStat');
  var dz=document.getElementById('importDZ');
  stat.style.color='#1565c0';dz.style.background='#e3f2fd';dz.style.borderColor='#1565c0';
  document.getElementById('scanTip').style.display='none';
  var ext=file.name.split('.').pop().toLowerCase();
  try{
    if(ext==='docx'||ext==='doc'){
      stat.textContent='\u23f3 Se proceseaza '+file.name+'\u2026';
      var t=await rdDocx(file);
      if(!t||t.trim().length<10)throw new Error('Fisier DOCX gol sau corupt.');
      document.getElementById('importDlgTxt').value=t.substring(0,12000);
      var found=curImpType==='cg'?parseCGImproved(t):parseIPTImproved(t);
      if(!found.length)found=parseUni(t);
      if(!found.length){stat.style.color='#e65100';stat.textContent='\u26a0 Text extras dar module nedetectate. Editati campul si apasati Detecteaza.';dz.style.background='#fff3e0';dz.style.borderColor='#ef6c00';return;}
      document.getElementById('modBody').innerHTML='';rowIdx=0;
      found.forEach(function(f){addRow(f.name,f.ore,f.nrEv||0);});
      ...
      stat.textContent='\u23f3 Se identifica tabelul din PDF\u2026';
      ...
      if(found.length){
        ...
        stat.style.color='#e65100';stat.textContent='\u26a0 PDF scanat sau tabel nedetectat \u2014 folositi instructiunile de mai jos.';
...
function parseCGImproved(raw){
  ...
  for(var i=si+2;i<lines.length;i++){if(/PROIECTAREA\s+DIDACTIC|REPARTIZAR|PLANUL\s+CALEND|COMPETENTE\s+SPECIFICE/i.test(lines[i])){ei=i;break;}}
  var skip=/^(nr\.?|modul|denumire|unitat|ore|total|evaluari|continut|administrarea|proiectarea|disciplin|luna|semestru)/i;
  for(var i=si;i<ei;i++){
...
function parseIPTImproved(raw){
  var lines=raw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length>0;});
  var found=[],si=0,ei=lines.length;
  for(var i=0;i<lines.length;i++){if(/REPARTIZAR[EA]/i.test(lines[i])){si=i+1;break;}}
  for(var i=si+2;i<lines.length;i++){if(/PROIECTAREA|COMPETENTE\s+SPECIFICE|OBIECTIVE/i.test(lines[i])&&i>si+3){ei=i;break;}}
  var skipRx=/^(nr\.?\s*(crt)?|repartiz|numarul\s+de\s+ore|contact\s+direct|lucrul\s+indiv|prelegeri\s*$|practica\s*\/|laborator\s*$|total\b)/i;
  for(var i=si;i<ei;i++){
    var ln=lines[i];if(skipRx.test(ln))continue;
    var cells=splitCells(ln);if(!cells||cells.length<3)continue;
    var nameCol=0;
    var c0num=parseInt(cells[0]);
    if(/^[IVXLCDM]{1,5}\.?$/.test(cells[0])||(/^\d{1,2}\.?$/.test(cells[0])&&c0num>=1&&c0num<=50)){nameCol=1;}
    if(nameCol>=cells.length)continue;
    if(/^\s*total\s*$/i.test(cells[nameCol]||''))continue;
    if(/^\d+$/.test((cells[nameCol]||'').trim()))continue;
    var allNum=cells.every(function(c){return/^\d+$/.test(c.trim());});if(allNum)continue;
    var name=(cells[nameCol]||'').replace(/^(\d{1,2}|[IVXLCDM]+)[\.\s)\-\u2013]+/i,'').replace(/\s*\(\d+\s*ore\)/i,'').trim();
    if(name.length<3||!/[a-zA-Z\u0103\u00e2\u00ee\u0219\u021b\u0102\u00c2\u00ce\u0218\u021a]/.test(name))continue;
    var nums=cells.slice(nameCol+1).map(function(c){var n=parseInt((c||'').replace(/\D/g,''));return isNaN(n)?0:n;}).filter(function(n){return n>0&&n<=300;});
    if(!nums.length)continue;
    var ore;
    if(nums.length>=3){var s=nums.slice(1).reduce(function(a,b){return a+b;},0);ore=(nums[0]===s)?nums[1]:nums[0];}
    else{ore=nums[0];}
    if(ore>0&&ore<=300)found.push({name:name,ore:ore,nrEv:0});
  }
  return found;
}
function parseUni(raw){
  var found=[],lines=raw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length>2;});
  var skip=/^(nr\.?|modul|denumire|ore|evaluari|eval|unitat|continut|total|sumar|\*{3,}|[-=|]{4,})/i;
...
async function rdDocx(file){
  if(typeof JSZip==='undefined')throw new Error('Import DOCX necesita conexiune (JSZip).');
  var ab=await file.arrayBuffer(),z=await JSZip.loadAsync(ab),xf=z.file('word/document.xml');
  if(!xf)return '';
  var xml=await xf.async('string');
  function nTxt(x){
    var pRx=/<w:p[ >][\s\S]*?<\/w:p>/g,pm;
    while((pm=pRx.exec(x))!==null){
      var t=pm[0].replace(/<w:br[^>]*>/g,' ').replace(/<w:t[^>]*>([^<]*)<\/w:t>/gi,'$1').replace(/<[^>]+>/g,'').replace(/\s+/g,' ').trim();
      if(t.length>0)return t;
    }
    return x.replace(/<w:br[^>]*>/g,' ').replace(/<w:t[^>]*>([^<]*)<\/w:t>/gi,'$1').replace(/<[^>]+>/g,'').replace(/\s+/g,' ').trim();
  }
...
function buildTests(ms,cfg){
  ...
  t.push({nr:i++,key:'ti_0',den:'Test Initial',tip:'Intrare',modIds:[],allMods:true,nrItemi:ic.nrItemi,timp:ic.timp,prag:ic.prag,bloom:{nc1:ic.nc1,nc2:ic.nc2,nc3:ic.nc3}});
  ms.forEach(function(m){
    ...
    if(m.nrEval>0){var sum=getTestCfg('Intermediar');t.push({nr:i++,key:'sm_'+m.id,den:toRoman(m.id)+'. '+m.name.substring(0,22)+' \u2014 Sumativa',tip:'Intermediar',modIds:[m.id],allMods:false,nrItemi:sum.nrItemi,timp:sum.timp,prag:sum.prag,bloom:{nc1:sum.nc1,nc2:sum.nc2,nc3:sum.nc3}});}
    oa+=m.ore;
    if(oa>=cfg.s){var np=Math.floor(oa/cfg.s);for(var s2=0;s2<np;s2++){var ic2=getTestCfg('Intermediar'),cv=ms.slice(0,ms.indexOf(m)+1).map(function(x){return x.id;});t.push({nr:i++,key:'int_'+ns,den:'Eval. intermediara nr.'+ns,tip:'Intermediar',modIds:cv,allMods:false,nrItemi:ic2.nrItemi,timp:ic2.timp,prag:ic2.prag,bloom:{nc1:ic2.nc1,nc2:ic2.nc2,nc3:ic2.nc3}});ns++;}oa=oa%cfg.s;}
  });
...
function buildTreeUI(cid,cb,ak){var el=document.getElementById(cid);if(!testsList.length){el.innerHTML='<div class="es"><div class="ei">⚡</div>Generati</div>';return;}var grp={Intrare:[],Tematic:[],Intermediar:[],Final:[]};testsList.forEach(function(t){if(grp[t.tip])grp[t.tip].push(t);});var ic={Intrare:'\ud83d\udd35',Tematic:'\ud83d\udfe2',Intermediar:'\ud83d\udfe3',Final:'\ud83d\udd34'},h='';Object.keys(grp).forEach(function(tip){var l=grp[tip];if(!l.length)return;h+='<div class="tg">'+ic[tip]+' '+tip+'</div>';l.forEach(function(t){var sel=t.key===ak;h+='<div class="ti'+(sel?' sel':'')+'" onclick="'+cb+'(\''+t.key+'\')"><span style="flex:1">'+t.den+'</span><span class="tin">'+t.nrItemi+'</span></div>';});});el.innerHTML=h;}
...
function selectMat(key){
  ...
  h+='<div class="brow" style="margin-top:10px"><button class="b bp" onclick="selectSbl(\''+key+'\');gt(\'sbl\')">→ Sablon</button><button class="b" onclick="exp(\'mat\')">↓ DOCX</button></div>';
...
function selectSbl(key){
  ...
  var h='<div style="background:'+col+';color:#fff;border-radius:8px;padding:10px 14px;margin-bottom:10px"><b style="font-size:13px">📋 SABLON \u2014 '+t.den+'</b><div style="font-size:10px;opacity:.9;margin-top:2px">'+t.nrItemi+' itemi \u00b7 '+pt+' pct \u00b7 '+t.timp+' min \u00b7 Prag '+t.prag+'%</div></div>';
...
function loadDemo(){document.getElementById('modBody').innerHTML='';rowIdx=0;document.getElementById('cC').value='Informatica_X';[['Recapitulare si consolidare',2,1],['Metode de descriere a limbajelor',4,1],['Vocabularul si sintaxa limbajului',5,1],['Tipuri de date simple',10,2],['Instructiuni de programare',25,3],['Elemente de Web Design',22,2]].forEach(function(d){addRow(d[0],d[1],d[2]);});sts();toast('\ud83d\udcc4 Demo Informatica cl.X','#2e7d32');}
addRow('',0,0);addRow('',0,0);addRow('',0,0);
sts();